...
function ParametresEduc({ data, persist, me, isAdminGeneral, role }) {
  ...
  return (
    <div style={{ display: "grid", gap: 16, maxWidth: 520 }}>
      ...
      <Card style={{ fontSize: 11, color: COLORS.textDim, fontFamily: "monospace" }}>
        Debug temporaire — rôle de connexion brut : "{role}" (longueur {(role || "").length}) · reconnu comme administrateur général : {isAdminGeneral ? "oui" : "non"}
      </Card>
...
function EducateurApp({ data, persist, role, me, onLogout }) {
  ...
  return (
    <Shell title="Espace éducateur" subtitle={`${me ? `${me.prenom} · ${me.role}` : role} · NEC Académie · Saison ${data.season.number}`} tabs={tabs}
      ...
      {tab === "parametres" && <ParametresEduc data={data} persist={persist} me={me} isAdminGeneral={isAdminGeneral} role={role} />}
    </Shell>
  );
}
