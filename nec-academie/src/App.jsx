...
function SeancesRecentes({ data, onEdit, onStats, allowedGroupIds }) {
  const recent = data.seances.filter((s) => !allowedGroupIds || allowedGroupIds.includes(s.groupId)).slice(0, 6);
...
function MesGroupes({ data, onOpenGroup, onEdit, onStats, allowedGroupIds }) {
  const visibleGroups = allowedGroupIds ? data.groups.filter((g) => allowedGroupIds.includes(g.id)) : data.groups;
  return (
    <div style={{ display: "grid", gap: 22 }}>
      <SeancesRecentes data={data} onEdit={onEdit} onStats={onStats} allowedGroupIds={allowedGroupIds} />
      {allowedGroupIds && allowedGroupIds.length === 0 && (
        <div style={{ color: COLORS.textDim, fontSize: 13 }}>Aucun groupe ne vous a encore été assigné. Contactez l'administrateur général.</div>
      )}
      {Object.keys(MAISON_META).map((mid) => {
        const groups = visibleGroups.filter((g) => g.maisonId === mid);
...
function AccueilEduc({ data, onOpenGroup, me, allowedGroupIds }) {
  const visibleGroups = allowedGroupIds ? data.groups.filter((g) => allowedGroupIds.includes(g.id)) : data.groups;
  const upcoming = visibleGroups.slice(0, 2);
  return (
    <div style={{ display: "grid", gap: 16 }}>
...
function Repertoire({ data, persist, onValidate }) {
  ...
  const supprimerEnfant = async (childId) => {
    const child = data.children.find((c) => c.id === childId);
    if (!child) return;
    if (!window.confirm(`Supprimer définitivement ${child.prenom} ${child.nom} du répertoire ? Cette action est irréversible (historique, tridents et badges de cet enfant seront perdus).`)) return;
    const next = { ...data, children: data.children.filter((c) => c.id !== childId) };
    await persist(next);
    // Retire aussi cet enfant du compte famille lié, s'il existe.
    try {
      const q = query(collection(db, "users"), where("childIds", "array-contains", childId));
      const snap = await getDocs(q);
      snap.forEach((d) => {
        const ids = (d.data().childIds || []).filter((id) => id !== childId);
        setDoc(d.ref, { ...d.data(), childIds: ids });
      });
            ...
            {filtered.map((c) => (
              <tr key={c.id} style={{ borderBottom: `1px solid ${COLORS.border}` }}>
                ...
                <td style={{ padding: 10, display: "flex", gap: 6 }}>
                  ...
                  <Btn variant="danger" onClick={() => supprimerEnfant(c.id)}>Supprimer</Btn>
                </td>
              </tr>
            ))}
          </tbody>
...
function GestionAcces({ data, persist }) {
  ...
  const toggleGroup = (id) => setGroupIds((prev) => prev.includes(id) ? prev.filter((x) => x !== id) : [...prev, id]);
  const toggleGroupFor = async (staffId, groupId) => {
    const next = JSON.parse(JSON.stringify(data));
    const s = next.staff.find((x) => x.id === staffId);
    const current = s.groupIds || [];
    s.groupIds = current.includes(groupId) ? current.filter((x) => x !== groupId) : [...current, groupId];
    await persist(next);
  };
  ...
  const ajouter = async () => {
    ...
    try {
      ...
      const next = JSON.parse(JSON.stringify(data));
      next.staff.push({ id: staffId, prenom: prenom.trim(), nom: nom.trim(), email: email.trim(), role, groupIds });
      await persist(next);
      setPrenom(""); setNom(""); setEmail(""); setPassword(""); setGroupIds([]);
    } catch (e) {
      setErr(messageFor(e.code));
    } finally { setBusy(false); }
  };
  ...
  return (
    <Card>
      ...
      <div style={{ color: COLORS.textDim, fontSize: 11.5, marginBottom: 12 }}>Réservée à l'administrateur général : donner, modifier ou supprimer les accès des éducateurs et administrateurs. Les groupes cochés définissent ce qu'un éducateur voit dans "Mes groupes" (le Répertoire, lui, reste visible pour tous).</div>
      <div style={{ display: "grid", gap: 8, marginBottom: 8 }}>
        {data.staff.map((s) => (
          <div key={s.id} style={{ padding: "8px 10px", background: COLORS.panelLight, borderRadius: 10 }}>
            ...
            {s.role !== "Administrateur général" && (
              <div style={{ marginTop: 8, display: "flex", flexWrap: "wrap", gap: 6 }}>
                {data.groups.map((g) => {
                  const active = (s.groupIds || []).includes(g.id);
                  return (
                    <button key={g.id} onClick={() => toggleGroupFor(s.id, g.id)} style={{ fontSize: 11, padding: "4px 8px", borderRadius: 999, border: `1px solid ${active ? COLORS.gold : COLORS.border}`, background: active ? COLORS.gold : "transparent", color: active ? COLORS.bgDeep : COLORS.textDim, cursor: "pointer" }}>
                      {g.name}
                    </button>
                  );
                })}
                {data.groups.length === 0 && <span style={{ fontSize: 11, color: COLORS.textDim }}>Aucun groupe créé pour l'instant.</span>}
              </div>
            )}
          </div>
        ))}
      ...
      <div style={{ borderTop: `1px solid ${COLORS.border}`, paddingTop: 10, display: "grid", gap: 8 }}>
        ...
        {role !== "Administrateur général" && (
          <div>
            <div style={{ color: COLORS.textDim, fontSize: 11.5, marginBottom: 4 }}>Groupes assignés :</div>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {data.groups.map((g) => {
                const active = groupIds.includes(g.id);
                return (
                  <button key={g.id} onClick={() => toggleGroup(g.id)} style={{ fontSize: 11, padding: "4px 8px", borderRadius: 999, border: `1px solid ${active ? COLORS.gold : COLORS.border}`, background: active ? COLORS.gold : "transparent", color: active ? COLORS.bgDeep : COLORS.textDim, cursor: "pointer" }}>
                    {g.name}
                  </button>
                );
              })}
              {data.groups.length === 0 && <span style={{ fontSize: 11, color: COLORS.textDim }}>Aucun groupe créé pour l'instant — importez d'abord vos licenciés, ou créez des groupes.</span>}
            </div>
          </div>
        )}
...
/* ---------------- Import Excel (licenciés) ----------------
   Lit un classeur Excel où chaque feuille correspond à un groupe
   (ex: "Jardin Aquatique") et contient un ou plusieurs sous-groupes,
   chacun introduit par une ligne titre du type :
     "Jardin Aquatique A (Mercredi 11h-11h45) Hugo"
   suivie d'une ligne d'en-tête (Nom, Prénom, Date naissance, email,
   Numéro) puis des lignes d'enfants, jusqu'à la ligne titre suivante.
   Les enfants qui partagent le même e-mail parent sont regroupés sous
   UN SEUL compte famille.
------------------------------------------------------------------- */
const IMPORT_SHEET_TO_MAISON = {
  "jardin aquatique": "jardin",
  "boutchou": "jardin",
  "dragon blanc": "blanc",
  "dragon jaune": "jaune",
  "dragon collège": "jaune",
  "dragon rouge": "rouge",
  "dragon argent": "rouge",
};

function parseTitleRow(t) {
  const m = String(t).trim().match(/^(.*?)\s*\((.*?)\)\s*(.*)$/);
  if (m) return { name: m[1].trim(), horaire: m[2].trim(), coach: m[3].trim() };
  return { name: String(t).trim(), horaire: "", coach: "" };
}
...
function maisonForSheet(sheetName) {
  const key = sheetName.trim().toLowerCase();
  return IMPORT_SHEET_TO_MAISON[key] || null;
}

function parseWorkbook(arrayBuffer) {
  const wb = XLSX.read(arrayBuffer, { type: "array", cellDates: false });
  const groups = []; // { sheet, name, horaire, coach, maisonId, children: [] }
  wb.SheetNames.forEach((sheetName) => {
    const maisonId = maisonForSheet(sheetName);
    if (!maisonId) return; // feuille hors périmètre, ignorée
    ...
    while (i < rows.length) {
      const row = rows[i] || [];
      const c0 = row[0];
      const restEmpty = row.slice(1).every((x) => x === null || x === undefined || String(x).trim() === "");
      if (c0 && String(c0).trim() && String(c0).trim() !== "Nom" && restEmpty && String(c0).trim().length > 3) {
        const { name, horaire, coach } = parseTitleRow(c0);
        current = { sheet: sheetName, name, horaire, coach, maisonId, children: [] };
        groups.push(current);
        i += 1;
      } else if (c0 && String(c0).trim() === "Nom") {
        i += 1;
        while (i < rows.length) {
          const r2 = rows[i] || [];
          const nom = r2[0] ? String(r2[0]).trim() : "";
          const empty = !nom && r2.slice(1).every((x) => x === null || x === undefined || String(x).trim() === "");
          if (empty) { i += 1; break; }
          if (nom && current) {
            const prenom = r2[1] ? String(r2[1]).trim() : "";
            const email = r2[3] ? String(r2[3]).trim().toLowerCase() : "";
            const age = computeAge(r2[2]);
            current.children.push({ nom, prenom, email, age });
          }
          i += 1;
        }
      } else {
...
function ImportLicencies({ data, persist }) {
  ...
  const onFile = async (e) => {
    ...
    if (!file) return;
    setFileName(file.name); setParseError(""); setParsedGroups(null); setReport(null);
    try {
      const buf = await file.arrayBuffer();
      const groups = parseWorkbook(buf);
      if (groups.length === 0) setParseError("Aucun groupe reconnu dans ce fichier (feuilles attendues : Jardin Aquatique, Boutchou, Dragon Blanc, Dragon Jaune, Dragon Rouge, Dragon Argent, Dragon Collège).");
      setParsedGroups(groups);
    } catch (err) {
      setParseError("Impossible de lire ce fichier Excel : " + err.message);
    }
  };

  const totalChildren = (parsedGroups || []).reduce((s, g) => s + g.children.length, 0);
  const familyMap = {};
  (parsedGroups || []).forEach((g) => g.children.forEach((c) => {
    if (!c.email) return;
    familyMap[c.email] = familyMap[c.email] || [];
    familyMap[c.email].push(c);
  }));
  const totalFamilies = Object.keys(familyMap).length;
  const noEmailCount = (parsedGroups || []).reduce((s, g) => s + g.children.filter((c) => !c.email).length, 0);
  ...
  const lancerImport = async () => {
    ...
    // 1) Groupes : on réutilise un groupe existant du même nom, sinon on le crée.
    const groupIdFor = {};
    parsedGroups.forEach((g) => {
      let existing = next.groups.find((x) => x.name.trim().toLowerCase() === g.name.trim().toLowerCase());
      if (!existing) {
        existing = { id: `imp-${g.name}`.toLowerCase().replace(/[^a-z0-9]+/g, "-"), maisonId: g.maisonId, name: g.name, horaire: g.horaire };
        next.groups.push(existing);
      } else {
        existing.horaire = g.horaire || existing.horaire;
      }
      groupIdFor[g.name] = existing;
    });

    // 2) Enfants : ajoutés s'ils n'existent pas déjà (même prénom+nom+groupe).
    const childrenToAdd = [];
    parsedGroups.forEach((g) => {
      const group = groupIdFor[g.name];
      g.children.forEach((c) => {
        const childId = `${c.prenom}-${c.nom}-${g.name}`.toLowerCase().replace(/[^a-z0-9]+/g, "-");
        const already = next.children.find((x) => x.id === childId);
        if (already) { logLines.push(`⏭️ ${c.prenom} ${c.nom} (${g.name}) déjà présent, ignoré.`); return; }
        const childRecord = {
          id: childId, prenom: c.prenom, nom: c.nom, age: c.age ?? 0,
          groupId: group.id, maisonId: group.maisonId, tridents: 0,
          badgesObtained: [], historique: [],
          settings: { notifications: true, email: c.email || "" },
        };
        next.children.push(childRecord);
        childrenToAdd.push({ ...childRecord, email: c.email });
      });
    });

    // 3) Comptes famille : un compte par e-mail, avec tous les enfants liés.
    const families = {};
    childrenToAdd.forEach((c) => {
      if (!c.email) return;
      families[c.email] = families[c.email] || [];
      families[c.email].push(c.id);
    });
    const emails = Object.keys(families);
    setProgress({ done: 0, total: emails.length });

    for (let idx = 0; idx < emails.length; idx++) {
      const email = emails[idx];
      const childIds = families[email];
      try {
        const tempPassword = "Nec-" + Math.random().toString(36).slice(2, 10);
        const uid = await createManagedAccount(email, tempPassword);
        await setDoc(doc(db, "users", uid), { role: "famille", childIds });
        try { await sendPasswordResetEmail(auth, email); } catch (e2) { /* pas bloquant */ }
        logLines.push(`✅ Compte créé pour ${email} (${childIds.length} enfant${childIds.length > 1 ? "s" : ""}), e-mail de définition du mot de passe envoyé.`);
      } catch (e) {
        if (e.code === "auth/email-already-in-use") {
          logLines.push(`⚠️ ${email} : un compte existe déjà — enfant(s) ajouté(s) aux données mais pas relié(s) automatiquement à ce compte (à faire manuellement dans Firestore si besoin).`);
        } else {
          logLines.push(`❌ ${email} : erreur (${e.code || e.message}).`);
        }
      }
      setProgress({ done: idx + 1, total: emails.length });
      await new Promise((r) => setTimeout(r, 250));
    }

    await persist(next);
    setReport({
      groupes: parsedGroups.length,
      enfantsAjoutes: childrenToAdd.length,
      comptesCrees: logLines.filter((l) => l.startsWith("✅")).length,
      lignes: logLines,
    });
    setRunning(false);
  };
  ...
  return (
    <Card>
      <div style={{ color: COLORS.text, fontWeight: 700, marginBottom: 4, fontFamily: "'Baloo 2', sans-serif" }}>Import des licenciés (Excel)</div>
      <div style={{ color: COLORS.textDim, fontSize: 11.5, marginBottom: 12 }}>
        Importe les groupes et enfants depuis votre fichier répertoire. Feuilles reconnues : Jardin Aquatique, Boutchou, Dragon Blanc, Dragon Jaune, Dragon Rouge, Dragon Argent, Dragon Collège. Les fratries (même e-mail) partagent un seul compte ; chaque parent reçoit un e-mail pour définir son mot de passe.
      </div>

      {!running && !report && (
        <>
          <input type="file" accept=".xlsx,.xls" onChange={onFile} style={{ color: COLORS.textDim, fontSize: 12.5, marginBottom: 10 }} />
          {parseError && <div style={{ color: BRAND.red, fontSize: 12.5, marginBottom: 8 }}>{parseError}</div>}
          {parsedGroups && parsedGroups.length > 0 && (
            <div style={{ background: COLORS.panelLight, borderRadius: 10, padding: 12, marginBottom: 12 }}>
              <div style={{ color: COLORS.text, fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Aperçu — {fileName}</div>
              <div style={{ color: COLORS.textDim, fontSize: 12.5, marginBottom: 8 }}>
                {parsedGroups.length} groupes · {totalChildren} enfants · {totalFamilies} comptes famille à créer
                {noEmailCount > 0 && ` · ${noEmailCount} enfant(s) sans e-mail (ne recevront pas de compte)`}
              </div>
              <div style={{ maxHeight: 220, overflowY: "auto", display: "grid", gap: 4 }}>
                {parsedGroups.map((g) => (
                  <div key={g.sheet + g.name} style={{ display: "flex", justifyContent: "space-between", fontSize: 11.5, color: COLORS.textDim }}>
                    <span>{g.name} <span style={{ opacity: 0.7 }}>({g.horaire})</span></span>
                    <span>{g.children.length} enfant{g.children.length > 1 ? "s" : ""}</span>
                  </div>
                ))}
              </div>
              <Btn onClick={lancerImport} style={{ marginTop: 12 }}>Confirmer l'import</Btn>
            </div>
          )}
        </>
      )}
      ...
      {running && (
        <div style={{ color: COLORS.textDim, fontSize: 13 }}>
          Création des comptes en cours… {progress.done} / {progress.total}
          <div style={{ background: COLORS.panelLight, borderRadius: 8, height: 8, marginTop: 8, overflow: "hidden" }}>
            <div style={{ background: COLORS.gold, height: "100%", width: `${progress.total ? (progress.done / progress.total) * 100 : 0}%`, transition: "width .2s" }} />
          </div>
          <div style={{ marginTop: 8, fontSize: 11.5 }}>Ne fermez pas cette page pendant l'import.</div>
        </div>
      )}

      {report && (
        <div>
          <div style={{ color: COLORS.text, fontSize: 13, fontWeight: 600, marginBottom: 6 }}>Import terminé</div>
          <div style={{ color: COLORS.textDim, fontSize: 12.5, marginBottom: 8 }}>
            {report.groupes} groupes traités · {report.enfantsAjoutes} enfants ajoutés · {report.comptesCrees} comptes famille créés
          </div>
          <div style={{ maxHeight: 260, overflowY: "auto", display: "grid", gap: 3, background: COLORS.panelLight, borderRadius: 10, padding: 10 }}>
            {report.lignes.map((l, i) => <div key={i} style={{ fontSize: 11, color: COLORS.textDim }}>{l}</div>)}
          </div>
          <Btn variant="ghost" style={{ marginTop: 10 }} onClick={() => { setReport(null); setParsedGroups(null); setFileName(""); }}>Importer un autre fichier</Btn>
        </div>
      )}
    </Card>
  );
...
function ParametresEduc({ data, persist, me, isAdminGeneral, role, rawProfile }) {
  ...
  return (
    <div style={{ display: "grid", gap: 16, maxWidth: 520 }}>
      ...
      {isAdminGeneral && <ImportLicencies data={data} persist={persist} />}
...
function EducateurApp({ data, persist, role, rawProfile, me, onLogout }) {
  ...
  const allowedGroupIds = isAdminGeneral ? null : (me?.groupIds || []);
  ...
  return (
    <Shell title="Espace éducateur" subtitle={`${me ? `${me.prenom} · ${me.role}` : role} · NEC Académie · Saison ${data.season.number}`} tabs={tabs}
      ...
      {tab === "accueil" && <AccueilEduc data={data} onOpenGroup={handleOpenGroup} me={me} allowedGroupIds={allowedGroupIds} />}
      {tab === "groupes" && !openGroup && !statsGroup && <MesGroupes data={data} onOpenGroup={setOpenGroup} onEdit={handleEdit} onStats={handleStats} allowedGroupIds={allowedGroupIds} />}
